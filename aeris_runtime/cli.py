...
def cmd_knowledge(args):
    ...
    if args.action == "stats":
        ...
        return 0
    print(json.dumps(knowledge_search(args.query, args.limit), ensure_ascii=False, indent=2))
    return 0
...
def build_parser():
    ...
    ch = s.add_parser("chat", help="Private engineering chat; only endpoint-policy-compliant local/trusted-LAN provider")
    ...
    co = s.add_parser("core", help="Verify or snapshot the read-only canonical Core cache")
    cos = co.add_subparsers(dest="action", required=True)
    cos.add_parser("verify")
    snap = cos.add_parser("snapshot")
