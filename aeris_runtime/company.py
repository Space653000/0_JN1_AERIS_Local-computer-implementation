...
def validate_company_manifest(path: Path = MANIFEST) -> CompanyStatus:
    ...
    try:
        ...
        if role_data.get("maturity") != "DEFINED_NOT_ALL_EXECUTABLE": errors.append("role registry must truthfully expose current maturity")
    ...
    if not str(privacy.get("local_data_cloud_egress","")).startswith("DENY"): errors.append("local data cloud egress must be DENY")
    try:
        maturity=json.loads(MATURITY.read_text(encoding="utf-8"))
        if maturity.get("product_stage") != data.get("product_stage"): errors.append("maturity/product stage mismatch")
    except Exception as exc: errors.append(f"maturity matrix unreadable: {exc}")
