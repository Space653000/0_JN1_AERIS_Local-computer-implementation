(() => {
  ...
  for(const id of ['project','product','transducer','lifecycle','risk','evidenceTier','standardsStrategy','title','requirement','假設','證據Needed']){
  ...
  async function dashboard(){
    ...
    $('runtimeMode').textContent=String(s.runtime_mode).toUpperCase();$('offlineState').textContent=s.local_provider_ready?'LOCAL PROVIDER 就緒':'LOCAL PROVIDER DEGRADED';
    $('modelName').textContent=s.local_model||'NOT_CONFIGURED';$('modelState').textContent=s.local_provider_ready?'就緒':'DEGRADED';
    ...
    $('assetCount').textContent=`${s.skill_count}/${std}/${know}`;$('assetDetail').textContent='技能 / 標準 metadata / 知識 docs';
    $('workCount').textContent=`${s.control.projects}/${s.control.tasks}/${s.workflow_run_count}`;$('workDetail').textContent='專案s / SQLite tasks / Workflow runs';
    ...
    $('operationsList').innerHTML=row('Machine / GPU',byName['Machine / GPU Qualification']?.reason,byName['Machine / GPU Qualification']?.state)+row('Watchdog',byName['Watchdog Recovery']?.reason,byName['Watchdog Recovery']?.state)+row('Offline',byName['Offline Continuity']?.reason,byName['Offline Continuity']?.state)+row('Expected runs',byName['Expected-run 健康狀態']?.reason,byName['Expected-run 健康狀態']?.state);
    $('auditState').textContent=byName['Audit Ledger']?.state||'UNKNOWN';
    $('trustList').innerHTML=row('Audit',byName['Audit Ledger']?.reason,byName['Audit Ledger']?.state)+row('Evidence',byName['Evidence Store']?.reason,byName['Evidence Store']?.state)+row('驗證',byName['驗證 Engine']?.reason,byName['驗證 Engine']?.state)+row('Product stage',maturity.product_stage||'UNKNOWN','儀表板 ≠ Truth');
