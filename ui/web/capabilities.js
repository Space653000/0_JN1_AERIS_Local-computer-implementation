(() => {
  ...
  async function refresh(){if(loading)return;loading=true;try{const matrix=await api('');roles=matrix.roles;
    ...
    document.getElementById('capCounts').innerHTML=Object.entries(matrix.maturity_counts).map(([k,v])=>`<span class="chip">${escape(k)}: ${escape(v)}</span>`).join('')+`<span class="chip">技能 ${matrix.total_executable_skills}</span><span class="chip">Methods ${matrix.total_methods}</span><span class="chip">Skill 黃金 ${matrix.total_golden_cases}</span><span class="chip">Role 黃金 ${matrix.total_role_golden_cases??'UNKNOWN'}</span><span class="chip">Negative ${matrix.total_negative_cases}</span><span class="chip">回歸 ${matrix.total_regression_cases}</span><span class="chip">契約框架 ${matrix.total_roles-matrix.maturity_counts.L0}</span><span class="chip">角色領域驗收 ${matrix.maturity_counts.L3}</span><span class="chip">角色實體驗收 ${matrix.maturity_counts.L4}</span>`;
