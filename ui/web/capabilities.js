(() => {
  ...
  section.innerHTML='<div class="section-title"><div><div class="section-kicker">Professional Capability Factory</div><h2>聲學工程能力矩陣</h2></div><span class="pill" id="capSync">等待真實評估</span></div><div class="panel"><p>FREE_LOCAL_BASELINE · L2 可執行本機能力／L3 限定範圍解析評估／L4 真實儀器、校正與專家驗證。Memory ≠ Evidence。</p><div id="capCounts" class="chips" aria-live="polite"></div><div id="capCoverage" class="list"></div><details><summary>檢查 100 席能力、覆蓋與已知弱點</summary><div id="capRows"></div></details></div>';
  ...
  async function refresh(){if(loading)return;loading=true;try{const matrix=await api('');roles=matrix.roles;
    ...
    document.getElementById('capCounts').innerHTML=Object.entries(matrix.maturity_counts).map(([k,v])=>`<span class="chip">${escape(k)}: ${v}</span>`).join('')+`<span class="chip">Skills ${matrix.total_executable_skills}</span><span class="chip">Methods ${matrix.total_methods}</span><span class="chip">Golden ${matrix.total_golden_cases}</span><span class="chip">Negative ${matrix.total_negative_cases}</span><span class="chip">Regression ${matrix.total_regression_cases}</span>`;
