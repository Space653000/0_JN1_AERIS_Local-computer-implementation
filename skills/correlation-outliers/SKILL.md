...
Exact affine relation y=2x+1.

Invoke with `python -m aeris_runtime.engineering.factory run-skill correlation-outliers --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/correlation-outliers.json`. Analytical, negative and repeated-run cases: `golden/engineering/Failure/correlation-outliers`.
