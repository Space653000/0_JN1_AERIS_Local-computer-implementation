...
A seven-sample shifted impulse establishes correlation lag sign and magnitude.

Invoke with `python -m aeris_runtime.engineering.factory run-skill gcc-phat-tdoa --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/gcc-phat-tdoa.json`. Analytical, negative and repeated-run cases: `golden/engineering/Array/gcc-phat-tdoa`.
