...
Nonoverlapping band power contains the coherent tone energy once.

Invoke with `python -m aeris_runtime.engineering.factory run-skill fractional-octave --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/fractional-octave.json`. Analytical, negative and repeated-run cases: `golden/engineering/DSP/fractional-octave`.
