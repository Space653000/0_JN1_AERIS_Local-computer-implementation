...
Equal energy at 1 and 3 kHz has a 2 kHz centroid; no MOS inference.

Invoke with `python -m aeris_runtime.engineering.factory run-skill psychoacoustic-descriptors --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/psychoacoustic-descriptors.json`. Analytical, negative and repeated-run cases: `golden/engineering/Speaker/psychoacoustic-descriptors`.
