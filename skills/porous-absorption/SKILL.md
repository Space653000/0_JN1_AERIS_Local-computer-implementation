...
Passive rigid-backed fibrous layer; 1 kHz reference computed independently using scalar complex arithmetic and the stated Delany-Bazley equations.

Invoke with `python -m aeris_runtime.engineering.factory run-skill porous-absorption --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/porous-absorption.json`. Analytical, negative and repeated-run cases: `golden/engineering/Speaker/porous-absorption`.
