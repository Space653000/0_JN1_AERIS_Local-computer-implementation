...
A valid product plan includes both transducer workstreams and preserves requirement coverage.

Invoke with `python -m aeris_runtime.engineering.factory run-skill product-system-plan --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/product-system-plan.json`. Analytical, negative and repeated-run cases: `golden/engineering/Product/product-system-plan`.
