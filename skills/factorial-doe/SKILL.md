...
Single-factor effect equals high minus low response.

Invoke with `python -m aeris_runtime.engineering.factory run-skill factorial-doe --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/factorial-doe.json`. Analytical, negative and repeated-run cases: `golden/engineering/Product/factorial-doe`.
