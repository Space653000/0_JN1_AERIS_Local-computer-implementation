...
Maximin exploration selects the farthest allowed unexplored point without hardware actions.

Invoke with `python -m aeris_runtime.engineering.factory run-skill experiment-optimization --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/experiment-optimization.json`. Analytical, negative and repeated-run cases: `golden/engineering/Product/experiment-optimization`.
