# local-audio-regression

Version: 1.0.0. Tool layer: FREE_LOCAL_BASELINE.

Held-out affine regression without train/validation ID leakage.

Invoke with `python -m aeris_runtime.engineering.factory run-skill local-audio-regression --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/local-audio-regression.json`. Analytical, negative and repeated-run cases: `golden/engineering/Microphone/local-audio-regression`.
