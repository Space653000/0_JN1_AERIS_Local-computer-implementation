# circuit-noise-budget

Version: 1.0.0. Tool layer: FREE_LOCAL_BASELINE.

Ideal uniform quantization variance is one LSB squared divided by 12.

Invoke with `python -m aeris_runtime.engineering.factory run-skill circuit-noise-budget --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/circuit-noise-budget.json`. Analytical, negative and repeated-run cases: `golden/engineering/Microphone/circuit-noise-budget`.
