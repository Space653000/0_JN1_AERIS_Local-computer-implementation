...
Z weighting is unity gain.

Invoke with `python -m aeris_runtime.engineering.factory run-skill frequency-weighting --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/frequency-weighting.json`. Analytical, negative and repeated-run cases: `golden/engineering/DSP/frequency-weighting`.
