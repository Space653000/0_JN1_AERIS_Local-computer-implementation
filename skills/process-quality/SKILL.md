...
Sample SD=sqrt(0.5); centered Ppk=3/(3*SD).

Invoke with `python -m aeris_runtime.engineering.factory run-skill process-quality --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/process-quality.json`. Analytical, negative and repeated-run cases: `golden/engineering/Failure/process-quality`.
