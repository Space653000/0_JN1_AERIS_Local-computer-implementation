...
A noiseless gain-of-two system has H=2 and unit coherence in excited bins.

Invoke with `python -m aeris_runtime.engineering.factory run-skill transfer-coherence --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/transfer-coherence.json`. Analytical, negative and repeated-run cases: `golden/engineering/DSP/transfer-coherence`.
