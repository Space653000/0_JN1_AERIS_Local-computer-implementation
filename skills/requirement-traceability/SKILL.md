...
One of two requirements has a test/evidence link; missing coverage remains visible.

Invoke with `python -m aeris_runtime.engineering.factory run-skill requirement-traceability --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/requirement-traceability.json`. Analytical, negative and repeated-run cases: `golden/engineering/Product/requirement-traceability`.
