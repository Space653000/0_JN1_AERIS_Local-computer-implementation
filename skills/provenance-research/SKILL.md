...
Local retrieval preserves document hash and rights classification.

Invoke with `python -m aeris_runtime.engineering.factory run-skill provenance-research --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/provenance-research.json`. Analytical, negative and repeated-run cases: `golden/engineering/Product/provenance-research`.
