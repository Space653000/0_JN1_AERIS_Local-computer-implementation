...
Contradictory evidence leaves the cause contested and generates a discriminating test.

Invoke with `python -m aeris_runtime.engineering.factory run-skill failure-hypotheses --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/failure-hypotheses.json`. Analytical, negative and repeated-run cases: `golden/engineering/Failure/failure-hypotheses`.
