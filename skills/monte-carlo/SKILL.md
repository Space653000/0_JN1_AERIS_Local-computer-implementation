...
Sum of independent normals has mean 11 and standard deviation 5.

Invoke with `python -m aeris_runtime.engineering.factory run-skill monte-carlo --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/monte-carlo.json`. Analytical, negative and repeated-run cases: `golden/engineering/Failure/monte-carlo`.
