...
Broadside in-phase sensors have normalized unit power.

Invoke with `python -m aeris_runtime.engineering.factory run-skill linear-array-pattern --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/linear-array-pattern.json`. Analytical, negative and repeated-run cases: `golden/engineering/Array/linear-array-pattern`.
