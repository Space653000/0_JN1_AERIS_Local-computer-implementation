...
Independent variance terms are 0.01 and 0.16.

Invoke with `python -m aeris_runtime.engineering.factory run-skill uncertainty-propagation --input PATH`. Inputs must match `input.schema.json`; unknown fields, nonfinite values and out-of-domain values are rejected.

Method: `methods/engineering/uncertainty-propagation.json`. Analytical, negative and repeated-run cases: `golden/engineering/Failure/uncertainty-propagation`.
